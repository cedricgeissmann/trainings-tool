...
class ChatUtil{
	...
	public static function getTeamMembers($username){
		$resultString = "";
		$res = DatabaseUtil::executeQuery("SELECT DISTINCT user.username, firstname, name FROM user LEFT JOIN role ON (user.username = role.username) WHERE tid IN (SELECT tid FROM role WHERE username = '$username') AND activate='1' AND user.username NOT IN ('$username')");
