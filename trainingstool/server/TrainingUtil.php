...
class TrainingUtil {
	...
	private static function getNotSubscribedPersons($trainingsID) {
		$resUsernames = DatabaseUtil::executeQueryAsJSON( "SELECT DISTINCT user.username AS username, user.firstname AS firstname, user.name AS name, training.id AS trainingsID FROM user INNER JOIN (role) ON (role.username=user.username) INNER JOIN (training) ON (training.teamID=tid) WHERE training.id = '$trainingsID' AND activate='1' AND NOT EXISTS (SELECT username FROM subscribed_for_training AS b WHERE trainingsID ='$trainingsID' AND user.username = b.username)" );
		return json_decode($resUsernames);
	}
	 ...
	 */
	 private static function getAdminOrTrainer(){
	 	$username = $_SESSION["user"]["username"];
	 	$res = DatabaseUtil::executeQuery("SELECT username FROM role WHERE (trainer='1' OR admin='1') AND username='$username'");
	...
	public static function getTraining() {
		$username = $_SESSION["user"]["username"];
		$res = DatabaseUtil::executeQueryAsJSON( "SELECT training.id AS id, training.date AS date, training.day AS day, training.time_start AS time_start, training.time_end AS time_end, training.teamID AS teamID, role.admin AS admin, role.trainer AS trainer, training.location AS location, training.type AS type, training.meeting_point AS meeting_point, training.enemy AS enemy, teams.need_reason AS need_reason
		FROM `training` INNER JOIN (role, teams) ON (training.teamID = role.tid AND training.teamID=teams.id) WHERE date>='" . date ( "Y-m-d" ) . "' AND username='$username' AND deleted='0' ORDER BY date ASC, time_start ASC" );
		$json_obj = json_decode( $res );
		foreach($json_obj as $item){
			$id = $item->id;
			$item->numberOfPlayers = self::getNumberOfPlayersForTraining($id);
			$item->subscribed = self::getSubscribedPersons($id, 1);
			$item->unsubscribed = self::getSubscribedPersons($id, 0);
			$item->notSubscribed = self::getNotSubscribedPersons($id);
			$item->notifications = self::loadNotificationListData($id);
			$item->adminOrTrainer = self::getAdminOrTrainer();
		}
	...
	public static function getTeamFilter($username){
		$res = DatabaseUtil::executeQueryAsJSON("SELECT * FROM teams WHERE id IN (SELECT tid FROM role WHERE username='$username')");
		return "{\"teams\": $res}";
	}
	...
	public static function loadNotificationListData($trainingsID){
		return json_decode(DatabaseUtil::executeQueryAsJSON("SELECT * FROM `training_notification` WHERE trainingID='$trainingsID'"));
	}
}
