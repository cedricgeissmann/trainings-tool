...
function unsubscribeHandler(element){
	...
	if (need_reason === 1) {		//TODO get the need_reason in the training data.
		...
		unsubscribe(id);
		var name = getSessionsUsername();		//TODO get name from cache, remove the linebreaks in the same function.
		name = removeLineBreaks(name);
		movePlayer(name, element, "unsubscribeList");
	}
}
...
/**
 * Renders the notification data that is attached to the triggering element, and displays it in a modal.
 * @param elem the triggering html element. This element needs the data attribute notification.
 */
function displayNotification(elem) {
	var notificationData = elem.data("notification");
	if (notificationData.length > 0) {
		var res = json2html.transform([{
			"notification": notificationData
		}], trainingNotificationTransform); //pack into its own JSON so the modal is only rendered once.
		if ($("#trainingNotificationModal").size() > 0) {
			$("#trainingNotificationModal").remove();
		}
		$(res).insertBefore("#removeNext");
		$("#trainingNotificationModal").modal("toggle");
		checkAdminAndTrainerWithCallback(setVisibilities);		//call this here because a new element is created. This element can hold elements that are only available to admins or trainers.
	}
}
 ...
 * @param elem the html element to which the handler gets attached.
 */
function addTrainingNotificationHandler(elem) {
	var element = $(elem);
...
/**
 * TODO: Load just as many trainings as can be displayed.
...
function renderTeamFilter(filterData){
	if (filterData.length >= 2) {
		var res = json2html.transform(filterData, teamFilterTransform);
		$("#teamFilter").append(res);
		addSidebarHandlers();
	} else{
...
/**
 * Removes all empty training information divs.
 */
function removeUnusedTrainingInformations() {
	$(".enemySpan:empty").parent().remove();
	$(".meetingPointSpan:empty").parent().remove();
	$(".locationSpan:empty").parent().remove();
...
function addTrainingPanelHandlers(){
	...
	addHandlerToElements(".removeTraining", "click", removeTrainingHandler);
	addHandlerToElements(".addTrainingPlan", "click", addTrainingPlanHandler);
	addHandlerToElements(".addTrainingNotification", "click", function() {
		addTrainingNotificationHandler(this);
	});
	//addHandlerToElements(".notification", "click", displayNotification);		//Deactivate, beause this is set after adding the notification data to the labels.
	addHandlerToElements(".changeEvent", "click", function() {
		loadChangeEventHandler($(this));
	});
}
 ...
 * @param notifications the SJON object which is rendered to the html element.
...
/**
 * Loads the participants for each training and adds them to the list of subscribed, unsubscribed or not subscribed persons.
 */
function loadPanelContent(){
	$(".numberOfPlayers").each(function(){
		var elem = $(this);
		var id = elem.data("id");
		getNumberOfPlayersForTraining(id, function(data){elem.html(data);});
	});
	$(".subscribeList").each(function(){
		var elem = $(this);
		var id = elem.data("id");
		getPersonList(id, 1, elem);
	});
	$(".unsubscribeList").each(function(){
		var elem = $(this);
		var id = elem.data("id");
		getPersonList(id, 0, elem);
	});
	$(".notSubscribedList").each(function(){
		var elem = $(this);
		var id = elem.data("id");
		getPersonList(id, -1, elem);
	});
...
$("document").ready(function() {
	...
	loadTeamFilterData();
	resizeArea("content");
	loadTeamListData();		//execute this on load, because the root element of this list allready exists in the html document.
	//HERE
});
